...
CLASS ltcl_integration_test IMPLEMENTATION.
  ...
  METHOD create_travel.
    ...
    DATA travels_in TYPE TABLE FOR CREATE zats_rv_aroy_u_travel\\travel.

    travels_in = VALUE #( (
                              %cid = 'AbhinavTest-01'
                              agencyid      = 070001   "Agency 070001 does exist, Agency 1 does not exist
                              customerid    = 1
                              begindate     = today
                              enddate       = today + 30
                              bookingfee    = 30
    ...
    MODIFY ENTITIES OF zats_rv_aroy_u_travel
      ENTITY travel
    ...
    SELECT * FROM zats_rv_aroy_u_travel WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
  ...
  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities = VALUE #( ( i_for_entity = 'zats_rv_aroy_u_travel' )
                              ) ).
  ENDMETHOD.
